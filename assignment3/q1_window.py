...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        ...
        for i in xrange(length):
            windowed_data.append((reduce(lambda x,y:x+y, sentence[i:2*window_size+1+i]),
                                  labels[i]))
    ### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(tf.nn.embedding_lookup(pretrained_embeddings,
                                                       self.input_placeholder),
    ...
    def add_prediction_op(self):
        ...
        # U = tf.get_variable(name='U', shape=(self.config.hidden_size, self.config.n_classes), dtype=tf.float32,
        #                     initializer=initializer)
        ...
        h = tf.contrib.layers.fully_connected(x, self.config.hidden_size)
        h_drop = tf.contrib.layers.dropout(h, keep_prob=self.dropout_placeholder)
        pred = tf.contrib.layers.fully_connected(h_drop, self.config.n_classes, activation_fn=None)
        ### END YOUR CODE
        return pred
