...
class SoftmaxModel(Model):
    ...
    def add_prediction_op(self):
        ...
        """
        ### YOUR CODE HERE
        self.W = tf.Variable(tf.zeros([self.config.n_features, self.config.n_classes]),dtype=tf.float32)
        self.b = tf.Variable(tf.zeros([self.config.n_classes]), dtype=tf.float32)
